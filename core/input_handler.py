...
class InputHandler:
    ...
    def handle_events(self, events, world, camera, grid, panel, ui,
    ...
    def handle_camera_movement(self, dt, camera):
        keys = pygame.key.get_pressed()

        if keys[pygame.K_a]:
            camera.move(camera.move_speed * dt, 0)
        if keys[pygame.K_d]:
            camera.move(-camera.move_speed * dt, 0)
        if keys[pygame.K_w]:
            camera.move(0, camera.move_speed * dt)
        if keys[pygame.K_s]:
            camera.move(0, -camera.move_speed * dt)